...
MOSTDISLIKED500K_PATH = os.path.join(ROOT_DIR,"data/processed/mostdisliked_500000.csv")
VIDEO_IDS_MOSTDISLIKED500K_LIST_PATH = os.path.join(ROOT_DIR,"data/interim/video_ids_mostdisliked_500000.txt")
COMMENTS_DOWNLOAD_PATH = os.path.join(ROOT_DIR,"data/processed/comments_json/")
COMMENTS_DOWNLOAD_PATH_CSV = os.path.join(ROOT_DIR,"data/processed/comments_csv/")

# VIDEO_DATA_PATH = './data/'

def run_script(start_from, should_convert_to_text_file):
    """
    This function will download comments for videos in the most_disliked_500000.csv
    
    Args:
        start_from (int):   
            The position of the video id in the most_disliked_500000.csv. Because the program
            will create a text file from the video ids in the csv, the position will be index + 1

        should_convert_to_text_file (bool): 
            If True, the program will open the most_disliked_500000.csv and extract all the video ids.
            The program expects the most_disliked_500000.csv file to exist in the same place as this 
            python script.
    ...
    How this script works:
        1.  The function will first open the most_disliked_500000.csv file from the path provided
        ...
        2.  Once the video ids are retrieved, the final_df DataFrame will be created. This DatFrame will
            ...
            as follows: disliked_videos_comments_{start_from}.csv. Hence the data that existed before this will 
    ...
    if (should_convert_to_text_file == 'y') or (should_convert_to_text_file == 'yes'):
        print("Opening mostdisliked_500000.csv")
        df = pd.read_csv(MOSTDISLIKED500K_PATH)['id']
        
        with open(VIDEO_IDS_MOSTDISLIKED500K_LIST_PATH, 'w') as file:
            for id in list(df):
                file.write(f"{id}\n")
        print("text file created. This file will be used in the future for downloading comments")
    
    # create the COMMENTS_DOWNLOAD_PATH folder. This will hodl all the json files
    os.makedirs(COMMENTS_DOWNLOAD_PATH, exist_ok=True)

    # create the COMMENTS_DOWNLOAD_PATH folder. This will hodl all the json files
    os.makedirs(COMMENTS_DOWNLOAD_PATH_CSV, exist_ok=True)
    
    comments_file_name = f'disliked_videos_comments_{start_from}.csv'
    ...
    with open(VIDEO_IDS_MOSTDISLIKED500K_LIST_PATH) as file:
...
if __name__ == "__main__":
    ...
    start_from, should_convert_to_text_file = None, None
    ...
    try:
        ...
        This argument ensures that the txt file is created from the most_disliked_50000.csv. 
        """
        print(msg)
        
    if start_from and should_convert_to_text_file:
        run_script(int(start_from), should_convert_to_text_file)